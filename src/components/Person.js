...
function LinkedinLink({ url }) {
  return (
    <a rel="noopener noreferrer" href={url} aria-label="Email" className="p-2">
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="16"
        height="16"
...
const RICHTEXT_OPTIONS = {
  renderNode: {
    ...
    [INLINES.HYPERLINK]: (node, children) => {
      return (
        <a className="text-sky-500 font-bold" href={node.data.uri}>
          {children}
        </a>
      );
    },
...
export default function Person({ person }) {
  const { name, socialTwitter, socialGithub, socialLinkedin, image, bio } =
    person;
  return (
    <div className="mt-8 mx-auto flex flex-col justify-center max-w-xs p-6 shadow-md rounded-xl sm:px-12 dark:bg-gray-900 dark:text-gray-100">
      <img
        src={image.url}
        alt={image.title}
...
Person.propTypes = {
  person: PropTypes.shape({
    ...
    socialTwitter: PropTypes.string.isRequired,
